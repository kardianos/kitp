...
- `client/src/filter/screen_preset.svelte.ts::SCREEN_TYPES` constant (4-tuple of layout names). Replaced by a `LAYOUTS` constant whose only role is to pick the correct renderer component. Code paths that today branch on `screen_type === 'kanban'` etc. branch on `layout` instead; the *number of branches is the same* but `screen_type` stops being authoritative metadata. (Gate 13 landed the rename: `SCREEN_TYPES` → `LAYOUTS`, type `ScreenType` → `Layout`, `readScreenType` → `readLayout`.)
- The Inbox's hardcoded `let viewMode = $state<'mine' | 'all'>('mine')` plus the two header buttons that toggle it. It becomes one item in a `toggle_group` on the seeded Inbox screen card (see §"Schema · Toggle groups"). A generic `<ScreenToggleGroups>` component reads the screen card's toggle groups and emits per-group predicate contributions. (Gate 13 dropped the state + buttons; the assignee scope is hardcoded `mine_only` in `InboxLayout.applyAssigneeScope` until `<ScreenToggleGroups>` lands and reads it from `toggle_groups.scope.mine_only` on the screen card.)
- The Inbox's hardcoded "agent-routed-to-me" branch in InboxLayout (the `authState.isAgent` early-return inside `refresh`). Becomes a separate seeded screen card (`Agent view`) with `flow_ref` set to the `status` flow and an extra predicate `routed_to_me=true`. The agent's session sees that screen because the seed registers it conditionally on `is_agent`. (Equivalent option: keep the conditional in code; the data-driven path is cleaner.)
- `ui/widgets/TerminalActionButton.svelte` as a standalone purpose-built widget. The "Close ▾" split becomes one cell in the generic `<TransitionBar>` component (see below). Same DOM affordance, same hotkey, no special prop named `terminalOptions`. (Deleted in Gate 7.)
- The would-have-been `'ideas'` value on the Layout type, the would-have-been per-row Accept / Reject buttons in InboxLayout, and any planned `screen_type='ideas'` route. The Ideas surface is one seeded screen card pointing at the `status` flow, with a `phase_scope` toggle group whose `triage` item is default-on and `active` / `terminal` items are default-off.
...
**Status:** Gates 1–13 are landed (kernel + comm).
...
11. **Refactor seed to template-stamping pattern.** Current seed/demo becomes "the template project." Add a stamping step (Go post-apply hook calling `project.stamp`) to produce the demo project. Seed the Ideas + Archive screen cards into the template. **Status (landed):** the install seed now creates a `Standard Project Template` project (`is_template=true`) carrying six status value-cards (one triage / three active / two terminal), the `Standard task` flow with twelve `flow_step` transitions, and six screens (`Inbox` / `Grid` / `Kanban` / `Project` / `Ideas` / `Archive`) with layouts, slugs, hotkeys, flow_refs, default_create_status overrides and toggle_groups. The demo project keeps its existing seed (own screens, own statuses, tasks); Gate 12 ships the client-side filter that hides templates from the user-facing project list. Stamping the demo from the template is deferred — a future gate may collapse the demo's seed into a `project.stamp` call. The Archive's `Last 30 days` filter card is also deferred until `closed_at` lands as a real attribute (the `phase_scope` toggle alone selects the row set today).
...
13. **Drop dead code.** `SCREEN_TYPES` constant, `viewMode` state on Inbox, `TerminalActionButton`, `screen_type` enum docs, the `screen.predicate` attribute proposal. **Status (landed):** `SCREEN_TYPES` → `LAYOUTS` rename done (the `Layout` type + constant survive only as the AdminScreens "+ Add screen" combobox source; ScreenHost dispatches on `layout` directly). `readScreenType` → `readLayout`. Inbox's `viewMode` state + Mine/All header buttons gone; the assignee scope is hardcoded `mine_only` until `<ScreenToggleGroups>` lands and reads it from the screen card's seeded `toggle_groups.scope.mine_only` item. `TerminalActionButton` was deleted in Gate 7; `NAV_CHORDS` in Gate 9. `screen_type` survives only as historical comments. The `screen.predicate` attribute proposal has no code references; only the considered-and-rejected mentions in this doc remain.
