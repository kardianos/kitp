...
<script lang="ts">
   ...
   *  preset cache scope so two project-layout screens in the same
  ...
  useShortcut('project_detail', '/', () => void focusSearch(), 'Focus search', {
    fireInInputs: false,
  });
  ...
  async function focusFirstRow(): Promise<void> {
    ...
    const row = document.querySelector<HTMLElement>(
      '[data-testid="project-tasks-list"] [tabindex="0"]',
    );
    if (row === null) return;
    selectedIndex = 0;
    row.focus();
