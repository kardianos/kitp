...
function makeUsers(): UserListWithRolesRow[] {
  return [
    ...
    },
    {
      ...
      roles: [{ role_name: 'member', scope_project_id: 11, scope_project_title: 'Beta Inc' }],
    },
    {
      id: 3,
...
describe('applyUserFilters', () => {
  ...
  it('substring-matches display_name case-insensitively', () => {
    const users = makeUsers();
    expect(applyUserFilters(users, 'alice', null).map((u) => u.id)).toEqual([1]);
    expect(applyUserFilters(users, 'ALICE', null).map((u) => u.id)).toEqual([1]);
    expect(applyUserFilters(users, 'an', null).map((u) => u.id)).toEqual([1]); // "Anderson"
    // Whitespace-only search treated as empty.
    expect(applyUserFilters(users, '   ', null).map((u) => u.id)).toEqual([1, 2, 3]);
  });

  it('roleFilter requires the user to hold at least one assignment with that role', () => {
    const users = makeUsers();
    expect(applyUserFilters(users, '', 'admin').map((u) => u.id)).toEqual([1]);
    expect(applyUserFilters(users, '', 'member').map((u) => u.id)).toEqual([1, 2]);
  ...
  it('search and roleFilter AND together', () => {
    ...
    expect(applyUserFilters(users, 'b', null).map((u) => u.id)).toEqual([2]);
    ...
    expect(applyUserFilters(users, 'alice', 'member').map((u) => u.id)).toEqual([1]);
  });
...
describe('assignRolePayload', () => {
  it('omits scopeProjectId when null (global scope)', () => {
    const payload = assignRolePayload(7, 'admin', null);
    expect(payload).toEqual({ userId: 7, roleName: 'admin' });
  ...
  it('includes scopeProjectId when provided', () => {
    const payload = assignRolePayload(7, 'member', 42);
    expect(payload).toEqual({ userId: 7, roleName: 'member', scopeProjectId: 42 });
  });

  it('treats scopeProjectId === 0 as a real id (not omitted)', () => {
    const payload = assignRolePayload(7, 'member', 0);
    expect(payload.scopeProjectId).toBe(0);
  });
});
    ...
    const users: UserListWithRolesRow[] = [
      {
        ...
        display_name: 'Alice',
        email: 'a@x',
        roles: [{ role_name: 'member', scope_project_id: 99 }],
      },
    ];
    const csv = buildRolesCsv(users);
