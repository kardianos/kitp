...
// VENDORED for web/: the upstream build imports the `trusted-types` package
// for these three names. kitp web does not depend on that package (and never
// enables Trusted Types), so the import is replaced with minimal local stubs
// that keep this declaration file self-contained. The runtime path is
// untouched (DOMPurify only emits a string when Trusted Types is off, which is
// our case — see web/src/util/markdown.ts).
type TrustedHTML = string;
type TrustedTypePolicy = unknown;
